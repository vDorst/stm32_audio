...
async fn main(spawner: Spawner) {
    ...
    let mut config = Config::default();
    {
        ...
        config.rcc.pll = Some(Pll {
            ...
            mul: PllMul::MUL192,
            divp: Some(PllPDiv::DIV4), // 25mhz / 25 * 192 / 4 = 84Mhz.
            divq: Some(PllQDiv::DIV4), // 25mhz / 25 * 192 / 7 = 48Mhz.
            divr: None,
        });

        // 25mhz / 25 * 384 / 5 = 76.8 Mhz.

        // 48Khz * 2 * 16bit = 1_536_000 bits/sec
        // FS = I2SxCLK / [(16*2)*((2*I2SDIV)+ODD))]

        // I2SxCLK= FS * (16*2)*((2*I2SDIV)+ODD)) = 48_000 * 16 * 2 * ((2* 12) + 1) = 38_400_000.
    ...
    let mut led_status = Output::new(p.PC13, Level::High, Speed::Low);

    let btn_key = Input::new(p.PA0, Pull::Up);
    ...
    info!("Read out I2S");
    for offset in [0x00, 0x08, 0x1c, 0x20] {
        let addr = unsafe { ptr.byte_add(offset) };
        let val = unsafe { *addr };
        println!("ADDR: {:08x} val {:08x}", addr, val);
    }

    // ADDR: 0x40003c00 val 0x0000_037c
    // ADDR: 0x40003c08 val 0x0000_0002
    // ADDR: 0x40003c1c val 0x0000_0e00
    // ADDR: 0x40003c20 val 0x0000_0008
    ...
    unwrap!(spawner.spawn(audio_receiver_task(receiver, i2s)));
